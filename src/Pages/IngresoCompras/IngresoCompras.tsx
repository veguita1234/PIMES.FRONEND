...
const IngresoCompras : React.FC = () => {
    const [openModalCompras, setOpenModalCompras] = useState(false);
    const [productosAgregadosCompras, setProductosAgregadosCompras] = useState<ProductoDTO[]>([]);
    const [documento, setDocumento] = useState("");
    const itemsPerPage = 5; 
    const [currentPageCompras, setCurrentPageCompras] = useState(1);
    const [fechaActualCompras, setFechaActualCompras] = useState<string>('');
    const [proveedorDataCompras, setProveedorDataCompras] = useState<ProveedorDTO | null>(null);
  ...
  const fetchProveedores = async (documento: string) => {
    try {
      const responseProveedor = await HttpModule.get(`/MaestrosProveedor/${documento}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });

      const data: ProveedorDTO = responseProveedor.data;

  
      if (data) {
        setProveedorDataCompras(data);
        setError(null); 
      } else {
        setError('Proveedor no encontrado');
        setProveedorDataCompras(null);
      }
    } catch (error) {
  ...
  const handleBuscarProveedor = () => {
    if (formData.numDoc.length > 0) {
      fetchProveedores(formData.numDoc); // Llama a la función con el valor de 'numDoc'
    } else {
  ...
  useEffect(() => {
    if (documento.length === 8) {
      fetchProveedores(documento);
    }
  }, [documento]);

  useEffect(() => {
    if (proveedorDataCompras) {
      setFormData({
        ...formData,
        nombres: proveedorDataCompras.nombres || '',
        apellidos: proveedorDataCompras.apellidos || '',
        razonSocial: proveedorDataCompras.razonSocial || '',
        direccion: proveedorDataCompras.ubicacion || '',
        correo: proveedorDataCompras.correo || '',
      });
      setError(null); 
    }
  }, [proveedorDataCompras]);

    const handleOpenModalCompras = () => {
        setOpenModalCompras(true);
      };
    
      // Manejar el cierre del modal
      const handleCloseModalCompras = () => {
        setFormData(initialFormData);
        setOpenModalCompras(false);
      };
    ...
    const handleSubmitCompras = async () => {
      ...
      try {
        ...
        if (response.status !== 200) {
          ...
          const errorMessage = response.data.mensaje || 'Error desconocido al agregar proveedor.';
          alert(` ${errorMessage}`);
          return;
        }
        ...
        setProveedores(prevProveedores => [...prevProveedores, data]);
        setFormData(initialFormData); // Limpia el formulario
        handleCloseModalCompras(); // Cierra el modal después de enviar
      } catch (error) {
    ...
    const emitirComprobante = async () => {
      ...
      if (productosAgregadosCompras.length === 0) {
          alert('No hay productos para enviar en el detalle.');
          return;
      }
  
      const pagarCabeceraDto = {
          NumDoc: formData.numDoc,
          Total: total,
      };
      ...
      try {
          ...
          // Crear la cabecera
          const responseCabecera = await HttpModule.post(`/MovimientosCtasPagarCabecera/${userId}`, pagarCabeceraDto, {
              headers: {
          ...
          if (idCabecera) {
              ...
              for (const producto of productosAgregadosCompras) {
                const cantidadCompras = cantidades[producto.idProducto] || 0;
                  const detalleComprasDto = {
                      ...
                      Cantidad: parseFloat(cantidadCompras.toFixed(2)),
                  };

  
                  const responseDetalle = await HttpModule.post(`/MovimientosCtasPagarDetalle/${userId}`, detalleComprasDto, {
                      headers: {
                          'Authorization': `Bearer ${token}`,
                          'Content-Type': 'application/json',
                      },
              ...
              alert("Compra emitida correctamente");
          } else {
...
useEffect(() => {
  ...
  const handleRemove = (idProducto: number) => {
    setProductosAgregadosCompras(prevProductos =>
  ...
  const [openAñadirProductosCompras, setOpenAñadirProductosCompras] = useState(false);

  const handleOpenAñadirProductoCompras = () => {
    setOpenAñadirProductosCompras(true);
  };
  const handleInputChangeProductoCompras = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleCloseAñadirProductoCompras = () => {

  setOpenAñadirProductosCompras(false);
};

  const handlePageChange = (direction: 'next' | 'prev') => {
    setCurrentPageCompras((prevPage) => {
      if (direction === 'next') {
        return prevPage + 1;
  ...
  const startIndex = (currentPageCompras - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedResults = results.slice(startIndex, endIndex);

  const handleAgregarProductoCompras = (producto: ProductoDTO) => {
    const productoExistente = productosAgregadosCompras.find(p => p.idProducto === producto.idProducto);
    
    if (!productoExistente) {
        setProductosAgregadosCompras([...productosAgregadosCompras, { ...producto, stock: 1 }]);
    } else {
        // Si deseas actualizar el stock en lugar de agregar
        setProductosAgregadosCompras(productosAgregadosCompras.map(p =>
...
const calculateTotal = () => {
  return productosAgregadosCompras.reduce((acc, producto) => {
...
}, [productosAgregadosCompras, cantidades, descuento]);
    ...
    return (
        <div style={{display:"flex",justifyContent:"center",padding:"30px",alignItems:"center"}}>
            <div style={{width:"60vw",borderRadius:"20px",border:"1px solid #E9ECF3",boxShadow:"4px 4px 10px rgba(0, 0, 0, 0.5)",backgroundColor:"white",display:"flex",alignItems:"center",flexDirection:"column",padding:"30px",gap:"3em"}}>
                <span style={{fontWeight:"bold",fontSize:"30px"}}>Ingreso de Compras</span>
                
                <div style={{width:"45vw",padding:"20px",display:"flex",flexDirection:"column",gap:"2em",border:"1px solid #E9ECF3",borderRadius:"10px"}}>
                    <span style={{fontWeight:"bold",fontSize:"15px"}}>Datos del Proveedor</span>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"2em"}}>
                        <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"12vw"}}>
                            ...
                            <select id="miSelector" name="opciones" style={{padding:"3px",fontSize:"13px"}}>
                                ...
                                <option value="opcion3">Carnet Extranjeria</option>
                                <option value="opcion4">Pasaporte</option>
                                <option value="opcion5">CDI</option>
                                <option value="opcion6">Doc.Iden.Pais.Residencia.no.D</option>
                                <option value="opcion7">Tax.Identif.Number.TIN.Doc.Trib.PP.NN</option>
                                <option value="opcion8">Identification.Number.IN.Doc.Trib.PP.JJ</option>
                            </select>
                        </div>
                        ...
                        <button onClick={handleBuscarProveedor}  style={{marginTop:"2.5vh",height:"3.2vh"}}><IoMdSearch /></button>
                        </div>
                        <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"8vw"}}>
                            <span style={{height:"1.5vh"}}></span>
                            <button onClick={handleOpenModalCompras} style={{fontFamily:"Segoe UI",fontSize:"12px",backgroundColor:"#4680FF",border:"1px solid #4680FF",color:"white",padding:"3.6px"}}>Nuevo Proveedor</button>
                        </div>
                        </div> 
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"36.8vw"}}>
                            ...
                            <input name="nombres" value={ proveedorDataCompras?.nombres} onChange={handleInputChange} style={{padding:"3px",fontSize:"13px",width:"36.2vw"}} />
                        </div>

                        <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"36.8vw"}}>
                            <label style={{fontFamily:"Segoe UI",fontSize:"12px"}}>Razón Social</label>
                            <input name="razonSocial" value={proveedorDataCompras?.razonSocial} onChange={handleInputChange}  style={{padding:"3px",fontSize:"13px",width:"36.2vw"}} />
                        </div>

                        <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"36.8vw"}}>
                            <label style={{fontFamily:"Segoe UI",fontSize:"12px"}}>Dirección</label>
                            <input name="direccion" value={proveedorDataCompras?.ubicacion} onChange={handleInputChange}  style={{padding:"3px",fontSize:"13px",width:"36.2vw"}} />
                        </div>

                        <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"36.8vw"}}>
                            <label style={{fontFamily:"Segoe UI",fontSize:"12px"}}>Correo Electrónico</label>
                            <input name="correo" value={proveedorDataCompras?.correo} onChange={handleInputChange}  style={{padding:"3px",fontSize:"13px",width:"36.2vw"}} />
                        </div>

                        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)"}}>
                            <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"12vw"}}>
                                <label style={{fontFamily:"Segoe UI",fontSize:"12px"}}>Fecha de Emisión</label>
                                <input value={fechaActualCompras} readOnly style={{padding:"3px",fontSize:"13px"}} />
                            </div>
                            <div style={{display:"flex",flexDirection:"column",gap:"0.4em",width:"12vw"}}>
                ...
                <div style={{width:"45vw",padding:"20px",display:"flex",flexDirection:"column",gap:"2em",border:"1px solid #E9ECF3",borderRadius:"10px"}}>
                    ...
                    <button onClick={handleOpenAñadirProductoCompras} style={{ width: "15vw", height: "4vh", backgroundColor: "#A2C76A", border: "1px solid #A2C76A" }}>+ Añadir Producto</button>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        ...
                        <tbody style={{backgroundColor:"white"}}>
                            {productosAgregadosCompras.map((producto) => (
                ...
                <Dialog open={openModalCompras} onClose={handleCloseModalCompras}>
                  ...
                  <DialogActions>
                    <Button onClick={handleCloseModalCompras} color="primary">Cancelar</Button>
                    <Button onClick={handleSubmitCompras} color="secondary">Aceptar</Button>
                  </DialogActions>
                  </Dialog>

                  {/*Dialog de Añadir Producto*/}
                  <Modal
                    open={openAñadirProductosCompras}
                    onClose={handleCloseAñadirProductoCompras}
                    ...
                    >
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Input
                          ...
                          onChange={handleInputChangeProductoCompras}
                          value={searchTerm}
                          fullWidth
                          disabled={loading}
                          sx={{
                          ...
                          {paginatedResults.map((result, index) => (
                            <ListItem key={index} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              ...
                              <Button
                                variant="contained"
                                color="primary"
                                onClick={() => handleAgregarProductoCompras(result)}
                                disabled={productosAgregadosCompras.some(p => p.idProducto === result.idProducto)}
                              >
                                {productosAgregadosCompras.some(p => p.idProducto === result.idProducto) ? "Agregado" : "Agregar"}
                              </Button>
                            </ListItem>
                          ))}
                        </List>
                      ...
                      <Box sx={{ display: 'flex',  mt: 0 ,justifyContent:"center"}}>
                        ...
                        <Typography style={{border:"1px solid",padding:"5px",alignContent:"center",fontSize:"12px",backgroundColor:"#1565C0",color:"white",fontWeight:"bold"}}>{`${currentPageCompras}`}</Typography>
                        <Button 
                      ...
                      <Button onClick={handleCloseAñadirProductoCompras}  color="secondary">
                        Cancelar
                      </Button>
                    </Box>
                  </Modal>
